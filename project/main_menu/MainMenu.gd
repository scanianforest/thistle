...
func _ready() -> void:
	...
	_world_name_input.focus_exited.connect(
		func(): _world_name_input.text_submitted.emit(_world_name_input.text)
	)
	_world_name_input.text_submitted.connect(_on_world_name_submitted)

	_start_button.text = (
		"CREATE" if not GameDataFileAccess.save_exists(_world_name_input.text) else "CONTINUE"
	)
