...
func face(direction: Vector2) -> void:
	...
	var directed_position: Vector2 = get_parent().global_position + half_grid_offset

	# Calculate tile coordinates and convert back to world position centered on tile
	var tile_coord = Vector2i(
		floor(directed_position.x / _grid_size.x), floor(directed_position.y / _grid_size.y)
	)
